...
class CardSetsFragment : BaseFragment<FragmentCardSetsBinding, CardSetsViewModel>(),
    ...
    private lateinit var cardSetAdapter: CardSetAdapter
    ...
    override fun onCreateOptionsMenu(menu: Menu, inflater: MenuInflater) {
        ...
        colorMenuItem(menu, getResColorInt(context!!, R.color.white))
            ...
            run {
                when (evt) {
                    ...
                    is CardSetsEvents.ShowExportSuccess -> {
                        ...
                        actionMode?.finish()
                        actionMode = null
                        val snackbar = Snackbar.make(mDataBinding.root, evt.message, Snackbar.LENGTH_INDEFINITE)
    ...
    private fun showConfirmDeleteCardSetDialog() {
    ...
    private fun showCardSetDeleteErrorDialog() {
        InfoDialog(context!!,
    ...
    private fun doFileOperationWithPermission() {
        if (export) {
            doExport()
        } else {
            doFileSelection()
    ...
    private fun doFileSelection() {
        val intent = Intent(Intent.ACTION_GET_CONTENT)
        intent.type = "*/*"
        startActivityForResult(Intent.createChooser(intent, "Select File"), RQ_SELECT_FILE)
    }

    private fun loadJSONFromUri(uri: Uri, context: Context): String? {
        val json: String?
        try {
