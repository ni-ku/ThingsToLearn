...
class QuestionCreateDialog(
    ...
    private val editParams: QuestionEditParams?
) : BaseDialog<DialogQuestionCreateBinding, QuestionCreateViewModel>() {

    companion object {
        val TAG: String = QuestionCreateDialog.toString()
    }
        ...
        mViewModel.mEvents.observe(this, Observer { evt ->
            run {
                when(evt) {
                    ...
                    is QuestionCreateEvents.SetCursorToPosition -> {
                        setCursorToPosition()
                    }
                }
            }
    ...
    private fun setCursorToPosition() {
        mDataBinding.etQuestion.postDelayed({
            mDataBinding.etQuestion.setSelection(mDataBinding.etQuestion.text!!.length)
        }, 20)
    }
}
